...
FUNDEC		:	typespec ID '('{
				 if (Search($2,level,0)==NULL){
					...
					off=2;//set to 2 now need it for our sp and base pointer 
                                        maxoffset=off;//set maxoffset 
				}
				else{
					//yyerror($2);
					yyerror("Symbol already used ");
...
exprstmt	:	expression ';' { printf(" test exprstmt\n");
...
writestmt	:	WRITE expression ';' { $$=ASTCreateNode(WRITESTMT);
		...
		|	WRITE STRING ';' { $$=ASTCreateNode(WRITESTMT);//added rule for string 
					$$->stringVal=CreateTempL();
					$$->name = $2; 
			}
		;
...
%% /*end rules */
//will now read -d our -o and will write to file specified 
