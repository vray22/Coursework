...
/*
Check length of parameter list and function call
return 0 if not 
*/
int CompareFormals(ASTnode *fparms, ASTnode *name){

//printf("IN CF \n");
	if((fparms == NULL) && (name == NULL)){return (1); }
	if((fparms == NULL) || (name == NULL)){return (0); }
	if(fparms->istype == name->istype ) return (CompareFormals(fparms->next,name->next));
	return (0);  
}
