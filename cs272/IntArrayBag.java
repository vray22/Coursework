...
//Lab3
//Feb 23 2017
//I will add methods equal, removeall, tostring, and intersection to the class
//CS272
...
public class IntArrayBag implements Cloneable {
	...
	/**
	 * Create a new bag that contains all the elements from the parameter.
	 * 
	 * @param object
	 *            
	 * 
	 *          
	 * @precondition Neither b1 nor b2 is null, and b1.getCapacity( ) +
	 *               b2.getCapacity( ) &lt;= Integer.MAX_VALUE. and both are same size
	 * @return true or false both bags are equal 
	 * @exception NullPointerException. Indicates
	 *                that one of the arguments is null.
	 * @exception OutOfMemoryError
	 *                Indicates insufficient memory for the new bag.
		...
		if (this.manyItems == candidate.manyItems) {
			for (int i = 0; i < manyItems; i++) {
				for (int j = 0; j < manyItems; j++) {
					if (this.data[i] == candidate.data[j]) {
						if (this.countOccurrences(this.data[i]) != candidate
								.countOccurrences(this.data[i])){
							return false;
						}
					}// end inner if
				}// end inner for
	...
	/**
	 * Create a new string with the elements of the bag which calls the function
	 * 
	 *           
	 * @precondition so bag can't be null if it is you will get null pointer exception 
	 *               
	 * @return string with elements of bag
	 * @exception NullPointerException. Indicates
	 *                that one of the arguments is null.
	 * @exception OutOfMemoryError
	 *                Indicates insufficient memory for the new bag.
	...
	public String toString() {
		String elements = "{";
		for (int i = 0; i < this.manyItems-1; i++) {
				elements = elements.concat(this.data[i] + "," + " ");
		}
		if(this.manyItems != 0)
			elements = elements + this.data[this.manyItems-1];
	...
	/**
	 * will remove all elements from one bag that are in the other bag which called the method 
	 * 
	 * @param removed:
	 *                  has to be of type IntArrayBag
	 *            
	 * @precondition Neither b1 nor b2 is null, and b1.getCapacity( ) +
	 *               b2.getCapacity( ) &lt;= Integer.MAX_VALUE.
	 * @return true or false if all elements have been removed that are in bag1 from bag2
	 * @exception NullPointerException. Indicates
	 *                that one of the arguments is null.
	 * @exception OutOfMemoryError
	 *                Indicates insufficient memory for the new bag.
	...
	/**
	 * Create a new bag that contains the elements that occur in both bag1 and bag2.
	 * 
	 * @param b1
	 *            IntArrayBag one 
	 * @param b2
	 ...
	 * @precondition Neither b1 nor b2 is null, and b1.getCapacity( ) +
	 *               b2.getCapacity( ) &lt;= Integer.MAX_VALUE.
	 * @return the new bag that has been created 
	 * @exception NullPointerException. Indicates
	 *                that one of the arguments is null.
	 * @exception OutOfMemoryError
	 *                Indicates insufficient memory for the new bag.
	...
	public static IntArrayBag intersection(IntArrayBag b1, IntArrayBag b2){
		boolean[] isFoundArray = new boolean[b2.manyItems];// Create lookup table for b2, array of b2.manyItems boolean type, initialize all to false
	    IntArrayBag intersecBag = new IntArrayBag(); // Create new bag empty
		for(int i = 0; i < b1.manyItems; i++){// Go through b1, do for loop
			for(int j = 0; j < b2.manyItems; j++){// Go through b2, do for loop
				if((b1.data[i] == b2.data[j]) && (isFoundArray[j] == false)){// if(b1[i] == b2[j] && isFondArray[j] == false{ add item to new bag; set isFoundArray[j] = true;
					intersecBag.addMany(b2.data[j]);
					isFoundArray[j] = true; 
				}//end if
			}//end inner for
		}//end outer for 
