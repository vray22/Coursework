...
ISR(TIMER1_COMPA_vect){
  ...
  //code for light stuff goes here 
   ...
   if(i < 25){
    ...
    light_Array[i] = l + r; //fills array until it is full 
    i++;
   
   } else { //array is full and it will start refilling it at index zero
...
void loop()
{
   ...
   l = readAD(4);//get light input for left side 
   //Serial.print("left light = ");
   //Serial.println(l, DEC);

    r = readAD(0);//get light input for right side 
   //Serial.print("right light = ");
   //Serial.println(r, DEC);

   delay(250);
    
   if (l > r && parkFlag == 0 ){//won't move if park flag 1, only set to one if there is park command 
      if (l-r > 3){   //check there is even enough sunlight to move 
          moveWest();
      }
   } 

   if(l < 3 && r < 3){//if both lights below threshold then it will return east if switch clicked
      switch2();
   }
   ...
   if (!strcmp(cmd,"reset")) {
      ...
      parkFlag = 0;
      resetPanel();
      memset(light_Array, 0, 25);//reset array to zero 
      ...
      parkFlag = 1;//park flag becomes one shouldn't track anymore 
      
   } else if (!strcmp(cmd,"query")) {
      Serial.println("Do query!");
      for(int i = 0; i < 25; i++){//will print values inside array starting at index 0
